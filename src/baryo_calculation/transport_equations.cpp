...
namespace BSMPT
{
    namespace Baryo
    {
                                        ...
                                        const double mChi_inp)
        {
            ...
            mChi=mChi_inp;
            init(vw_input,vev_critical_input,vev_symmetric_input,TC_input,modelPointer_input);
        }
                                        ...
                                        std::shared_ptr<Class_Potential_Origin> &modelPointer_input)
        {
            ...
            for (size_t i = 0; i < vev_critical.size(); i++)
                TestNorm += std::pow(modelPointer->get_vevTreeMin(i) - vev_critical.at(i), 2);
            ...
            for (size_t i = 0; i < vev_symmetric.size(); i++)
                TestNorm += std::pow(modelPointer->get_vevTreeMin(i) - vev_symmetric.at(i), 2);
            if (TestNorm > CompareNorm)
            {
                for (auto symmetry : modelPointer->SignSymmetries)
                {
            ...
            for (size_t i = 0; i < MinimumPlane.size(); i++)
                TestNorm += std::pow(modelPointer->get_vevTreeMin(i) - MinimumPlane.at(i), 2);
            if (TestNorm > CompareNorm)
            {
                for (auto symmetry : modelPointer->SignSymmetries)
                {
        ...
        double transport_equations::get_W_mass(const std::vector<double> &vev, const double &T) const
        {
            ...
            for (size_t i = 0; i < modelPointer->get_NGauge(); i++)
            {
                ...
                {
                    if (std::abs(res.at(i) - res.at(j)) <= 1e-5)
                        nrepeat[i]++;
                }
            }
        ...
        std::vector<double> transport_equations::get_top_mass_and_derivative(const std::vector<double> &vev) const
        {
            ...
            for (size_t i = 1; i <= modelPointer->get_NHiggs(); i++)
            {
                ...
                if (topmasssquared == -1)
                    topmasssquared = restmp.at(postop);
            ...
            const struct GSL_integration_mubl &params)
        {
            ...
            if (stepsize_initial == 0)
                stepsize_initial = -1e-7;
            ...
            for (size_t i = 0; i < dim; i++)
            {
                if (std::abs(parEnd[i]) <= std::pow(10, -16))
                    parEnd[i] = 0;
            }
            ...
            if (std::isnan(parEnd.at(0)))
            {
                ...
                for (size_t i = 0; i < parEnd.size(); i++)
